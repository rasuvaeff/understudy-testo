...
**Reset выполняется после вашего teardown `#[AfterTest]`.** `finally`
интерцептора стоит снаружи lifecycle-интерцептора, а лог вызовов удерживает
каждое возвращённое значение до этого reset — то есть значение, которое дубль
вернул, всё ещё удерживается, пока идёт ваш teardown. Для значения, владеющего
OS-ресурсом — стрим, соединение, лок, — ресурс всё ещё занят: forwarding-дубль,
возвращавший настоящие файловые стримы, ронял удаление каталога в teardown с
«Directory not empty» — только на Windows, потому что POSIX удаляет открытые
файлы. Такой дубль стройте lean (`Understudy::lean($double)` хранит вызовы,
но не возвращённые значения; understudy 0.4+) либо стройте и используйте
внутри `Understudy::scope()`, который сбрасывает контекст до teardown.
...
- **Одно ожидание на вызов.** Стаб `when()` и `expect()` на в точности одном
  и том же вызове не сочетаются, и движок с 0.3.0 отклоняет вторую
  регистрацию сразу — `ConflictingExpectation`; на старых движках пара
  деградировала молча (диспатч забирало объявленное позже, раннее теряло
  смысл). Идиома в любом случае одна: вешайте `->returns()` на сам `expect()`
  или `->times()` на `when()`.
...
«зачем настраивали, если он не нужен?». По умолчанию выключено, и точечной
формы у него нет: `Understudy::strict($double)` — это строгий **диспатч**
(«упасть на любом вызове, который не поймало ни одно ожидание») и о настроенном
и ни разу не вызванном стабе не говорит ничего. Аналог строгих стабов на один
дубль — `when(…)->times(n)`, который падает, когда счёт не сошёлся.
...
которые ассертят сами, сохраняют тот вердикт, который заслужили, а **тест, не
создавший ни одного дубля, адаптер не трогает вовсе**: для него ничего не
записывается, счётчик ассертов остаётся его собственным, вердикт раннера
остаётся в силе.

**Здесь верификация идёт после вашего teardown, а под PHPUnit — до него.** Этот
адаптер верифицирует в интерцепторе, снаружи `#[AfterTest]`; трейт PHPUnit — в
`assertPostConditions()`, который раннер вызывает *до* `tearDown()`. Ни то, ни
другое не неверно, но тест, чьё ожидание закрывает сам teardown, здесь пройдёт,
а там упадёт. `reset()` в обоих случаях выполняется после teardown.
...
Адаптер регистрирует интерцептор тестов и в конце каждого теста вызывает
`verifyAll()` и `reset()` движка. Он не исполняет код сверх того, что сам тест
запускает, и ничего не пишет.
