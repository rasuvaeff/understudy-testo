...
Тесты `#[TestInline]` и бенчмарки намеренно находятся вне области действия
перехватчика. Дубли нужно создавать в обычных тестах `#[Test]`: inline-кейсы
предназначены для чистых детерминированных таблиц и этим плагином не
верифицируются и не сбрасываются.
