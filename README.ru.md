...
но не возвращённые значения) либо стройте и используйте
...
- **Одно ожидание на вызов.** Стаб `when()` и `expect()` на в точности одном
  и том же вызове не сочетаются, и движок отклоняет вторую регистрацию
  сразу — `ConflictingExpectation`: диспатч забрало бы объявленное позже, а
  раннее молча потеряло бы смысл. Идиома одна: вешайте `->returns()` на сам
  `expect()` или `->times()` на `when()`.
